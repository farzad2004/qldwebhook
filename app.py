...
# APP_KEY = '2vxsimcvqyj99pz'
# APP_SECRET = 'ntwalv1djb0uerj'
...
def oauth_callback():
    ...
    # process_user(uid)
    token = 'Gq8-0VIcraAAAAAAAACh3NVW7B-8XDVBBbjbozCkhnlrhkM5rQDDyY8Sjn5ChyZr'

    dbx = dropbox.Dropbox('Gq8-0VIcraAAAAAAAACh3NVW7B-8XDVBBbjbozCkhnlrhkM5rQDDyY8Sjn5ChyZr')
    account = dbx.users_get_current_account()._account_id_value
    redis_client.hset('tokens', account, token)
...
def process_user(account):
    ...
    while has_more:
        ...
        for entry in result.entries:
            ...
            dbx.files_upload(html, entry.path_lower[:-3] + '.html', mode=dropbox.files.WriteMode.overwrite)
